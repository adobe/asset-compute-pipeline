...
class Prepare{
    ...
    static createBaseDirectory() {
        ...
        //     work/
        //       <activationid>/   <- base
        const base = Prepare.getBaseDirectory();
    ...
    static async createDirectories(folderName, baseDirectory, pipeline = true) {
        // all relative to current directory,
        // inside openwhisk nodejs container this would be:
        //
        //    /nodejsAction/xyz123
        ...
        if (!baseDirectory) {
            baseDirectory = Prepare.getBaseDirectory();
        }
        ...
        await fse.mkdirs(directories.base);

        // TODO: remove it when we move away from the SDK completely
        if (!pipeline) {
            debug(`running in non-pipeline mode`);
            // all relative to current directory,
            // inside openwhisk nodejs container this would be:
            //
            //    /nodejsAction/xyz123
     ...
     */
    static async cleanupDirectories(directories, outputPath) {
