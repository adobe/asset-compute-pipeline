...
const Mimetype = require("./utils/type");
 ...
 */
class Rendition {
    constructor(instructions, directory, index = 0, isPipelineRendition = true) {
        this.instructions = instructions; // e.g. quality, dpi, format etc

        // to stay backwards compatible as we switch from fmt -> mimtype, 
        // automatically convert this inside the rendition class
        if (!this.instructions.fmt) {
            this.instructions.fmt =  mime.extension(this.instructions.type);
        }
        this.directory = directory;
        this.index = index;
        this.target = instructions.target;
        this.pipeline = isPipelineRendition; // to mark renditions as "pipeline" renditions
    ...
    async _detectContentType() {
        if (this.exists() && this.size() > 0) {
            const type = await Mimetype.detectContentType(this.path);
    ...
    get name() {
        const fmt =  this.instructions.fmt || mime.extension(this.instructions.type);
        return Rendition.renditionFilename(fmt, this.index);
    }
    ...
    /**
     * Checks if a rendition exists: 
     * - rendition path existence
     * - rendition url existence (assume if url is defined, transformer uploaded result)
     * TODO: specify in instructions/transformer manifest if it will upload or create a local rendition
     * instead of assuming based on whats defined
     */
    ...
    async metadata() {
        ...
        // return here for pipeline rendition
        if (this.pipeline) {
            return meta;
        }
        ...
        // TODO add test case for this, see NUI-1431
        // when porting over from asset-compute-sdk, existence of these two conditions do not affect test
    ...
    static forEach(renditions, outDirectory, isPipelineRendition = true) {
        return renditions.map((instructions, index) => {
            return new Rendition(instructions, outDirectory, index, isPipelineRendition);
        });
    }
}
