...
async function getImageMetadata(path) {
    ...
    if (path && (await fs.pathExists(path))) {
        ...
        try {
            console.log(`Getting metadata via exiftool for source file path: ${path}`);
            ...
            console.error(`error getting metadata via exiftool for source file path: ${path}`);
            console.error(error);
        }
        ...
        try {
            ...
            console.log('Getting metadata via exiftool for source file URL');
            console.log(`Running command: ${command}`);
            const stdout = child_process.execSync(command, {
                timeout: EXIFTOOL_TIMEOUT_MS
            });
            ...
            console.error('Getting metadata via exiftool for source file URL');
            console.error(error);
        }
    }
