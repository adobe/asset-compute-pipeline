...
async function getImageMetadata(path) {
    ...
    if (path && (await fs.pathExists(path))) {
        ...
        try {
            console.log(`Getting metadata via exiftool for source file path: ${path}`, command);
            ...
            console.error(`error getting metadata via exiftool for source file path: ${path}`, error);
        }

    } else if (typeof (path) === 'string' && validUrl.isHttpsUri(path)) {
        try {
            const command = `curl -s "${path}" | exiftool -json - `;
            console.log(`Getting metadata via exiftool for source file URL ${command}`);
            const stdout = child_process.execSync(command, {
                timeout: EXIFTOOL_TIMEOUT_MS
            });

            if (stdout) {
                const responseArray = JSON.parse(stdout);
            ...
            console.error('Getting metadata via exiftool for source file URL', error);
        }
    }
