...
class Storage {
    ...
    static extension(filename, mimeType) {
        let ext = '';
        if (filename) {
    ...
    static sourceFilename(source) {
        if (source.name) {
            return `${SOURCE_BASENAME}${Storage.extension(source.name, source.mimeType)}`;
        }
    
        if (source.url && validUrl.isUri(source.url)) {
            const basename = path.basename(new URL(source.url).pathname);
            return  `${SOURCE_BASENAME}${Storage.extension(basename, source.mimeType)}`;
        }
    
        return `${SOURCE_BASENAME}${Storage.extension(null, source.mimeType)}`;
    }
    ...
    static async getSource(paramsSource, inDirectory, disableSourceDownload) {
        ...
        const name = Storage.sourceFilename(paramsSource);
