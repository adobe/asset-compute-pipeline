...
async function download(asset, file) {
    try {
        ...
        throw new GenericError(err.message, `${Action.actionName()}_download`);
    }
}
...
async function upload(rendition) {
    ...
    if (!file) {
        throw new GenericError(`rendition ${rendition.id()} does not have a file path: ${file}`, `${Action.actionName()}_upload`);
    }
    ...
    try {
        ...
        if (err.message && err.message.includes('is too large to upload') || err.status === 413) {
            ...
            throw new GenericError(err.message, `${Action.actionName()}_upload`);
        }
    } finally {
