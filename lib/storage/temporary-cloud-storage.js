...
class TemporaryCloudStorage {
    ...
    /**
     * Initialize temporary cloud storage
     */
    async _init() {
        // optional authorization passed in for using specific azure blob storage instead of ootb storage
        if (this.azureAuth) {
     ...
     */
    async generatePresignURL(cloudPath, permissions = "rwd", expiryInSeconds=2592000){
        if(!this.aioLibFiles){
            await this._init();
        }
        console.log(`PresignedUrl, locationFilePath ${cloudPath}, permissions ${permissions}, expiry ${expiryInSeconds}` );
