...
class Metadata {
    ...
    constructor(source) {
        ...
        } else if (!source.name && !source.path && !source.mimetype) {
            throw new GenericError(`Provided source must contain one of 'name', 'path', or 'mimetype' fields`);
        }
        this._source = source;
    ...
    async extract() {
        ...
        if (this.isVideo() || this.isTextTranscriptable()) {
            ...
            if (!input) {
                throw new GenericError(`Image source must contain either 'url' or 'path' field for retrieving metadata`);
            }
    ...
    getExtension(source) {
        let ext;
        const assetName = source.name || source.path;
        try {
