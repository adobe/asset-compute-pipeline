...
class Mimetype {
    static async detectTypeUsingFileCommand(renditionPath) {
        const result = {};
        ...
        try {
            ...
            result.mime = Mimetype.normalizeMimetype(type.type) || DEFAULT_MIME_TYPE;
    ...
    /**
     * Make sure the mimetype we return can be understood by other libraries/software
     * @param {*} mimetype Input mimetype to normalize
     * @returns Normalized mimetype
     */
     ...
     */
    static async detectContentType(file) {
        const result = await Mimetype.detectTypeUsingFileCommand(file);
