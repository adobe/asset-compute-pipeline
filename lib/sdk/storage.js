...
async function getAsset(assetReference, directory, name, disableDownload) {
    ...
    if (process.env.WORKER_TEST_MODE) {
        ...
        if (disableDownload) {
            ...
            if (isDataUrlAndValid(assetReference.url)) {
...
// function to verify if a input is a data url
// throw SourceUnsupportedError if it is a data url but invalid
function isDataUrlAndValid(input) {
    if (URL.parse(input).protocol !== 'data:') {
        return false;
    } else if (!validDataUrl(input)) {
        throw new SourceCorruptError(`Value ${input} is not a valid data url`);
    }
    return true;
}
