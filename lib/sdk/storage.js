...
const { AssetComputeLogUtils, GenericError, SourceUnsupportedError, SourceCorruptError } = require('@adobe/asset-compute-commons');
...
async function getAsset(assetReference, directory, name, disableDownload) {
    ...
    validateSupportedUrl(assetReference.url);
...
/**
 * Validate url
 * Url can be either a https url or datauri, throws if invalid
 * @param {String} input 
 */
function validateSupportedUrl(input) {
    // throws here if basic URL validation fails
    const protocol = new URL(input).protocol;

    switch (protocol) {
    case 'data:':
        if (!validDataUrl(input)) {
            throw new SourceCorruptError(`${input} is not a valid data url`);
        }
        break;
    case 'https:':
        if (!validUrl.isHttpsUri(input)) {
            throw new SourceCorruptError(`${input} is not a valid https url`);
        }
        break;
    default:
        throw new SourceUnsupportedError(`${input} protocol is not supported`);
    }
}
