...
class PlanFinder {
    ...
    async formPlan(source, outputInstructions, currentPlan) {
        ...
        // get image metadata and check for "Orientation" property if this is for Sensei
        if (this.registeredTransformers.length === 1 && this.registeredTransformers[0].includes("SenseiTransformer")) {
            ...
            if (metadata.Orientation) {
                ...
                if (orientationTransformer) {
                    ...
                    plan.push({name: orientationTransformer, attributes: instructions});
                }
            }
        }
