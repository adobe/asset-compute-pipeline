...
class PlanFinder {
     ...
     */
    formPlan(source, outputInstructions, currentPlan) {
    ...
    * @param {Object} attributes custom attributes for this step
    ...
    async findBestPlan(source, outputInstructions) {
        ...
        const metadata = await this.getImageMetadata(source.path);
        // if image has Orientation metadata set image to sensei compliant orientation
        if (metadata.Orientation) {
            const tempPlan = new Plan();
            // make instructions for orientation
            const instructions = {input: source, output: { type: source.type }};
            tempPlan.add("workerCallback-worker-flite", instructions);
            plan = this.BFS(source, outputInstructions, tempPlan);
        }
        else {
            plan = this.BFS(source, outputInstructions);
        }
    ...
    async getImageMetadata(path) {
        ...
        if (path && (await fs.pathExists(path))) {
            ...
            try {
                ...
                console.log(`Could not probe image orientation: ${err}`);
                return metadata;
            }
        }
        return metadata;
