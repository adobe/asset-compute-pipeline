...
class Engine {
    constructor(params) {
        ...
        // start time
        this[INTERNAL].context.workerStartTime = new Date();

        // metrics
        // TODO: metrics handling should entirely move to context
        this[INTERNAL].context.metrics = this[INTERNAL].params.metrics || new AssetComputeMetrics(params);

        // rendition errors
        this[INTERNAL].context.renditionErrors = [];

        this[INTERNAL].context.processingStartTime = this[INTERNAL].params.times.gateway ?
            new Date(this[INTERNAL].params.times.gateway) :
            new Date(this[INTERNAL].params.times.process);

        this[INTERNAL].context.metrics.add({
            pipeline:true,
            startWorkerDuration: utils.durationSec(this[INTERNAL].context.processingStartTime, this[INTERNAL].context.workerStartTime),
