...
const {GenericError, AssetComputeEvents, AssetComputeMetrics, OpenwhiskActionName, Reason, ClientError, AssetComputeLogUtils, SourceUnsupportedError} = require('@adobe/asset-compute-commons');
// get utils
...
class Engine {
    ...
    async prepare(transformerContext){
        ...
        let protocol = '';
        if (inputUrl) {
            if (!validDataUrl(inputUrl) && !validUrl.isHttpsUri(inputUrl)) {
                throw new SourceUnsupportedError(`${inputUrl} must be a valid https url or datauri`);
            }
        ...
        if (input.sourceType === TransformerSourceType.URL) {
            ...
            } else if (inputUrl && protocol === 'data:') {
