...
class Engine {
    constructor(params) {
        ...
        this[INTERNAL].baseDirectory = createBaseDirectory(); // base directory for all transformer directories
        this[INTERNAL].transformerDirectories = []; // list of all the transformer directories
        this[INTERNAL]._tempCloudStorageFiles = []; // list of files created in temporary cloud storage to be removed during clean up
    ...
    //TODO refactor to move transformer outside of transformerContext
    async prepare(transformerContext) {
        ...
        transformerContext.directory = await createDirectories(`${this.stepCount}-${transformer.name}`, this[INTERNAL].baseDirectory);
        this[INTERNAL].transformerDirectories.push(transformerContext.directory);
    ...
    async getPresignedUrl(source) {
        ...
        if (source.startsWith("https://")) {
            ...
            // Return url
            this[INTERNAL]._tempCloudStorageFiles.push(presignUrl);
            return presignUrl;
        }
    }
    ...
    async cleanup(finalOutput) {
        ...
        const cleanupSuccess = await cleanupDirectories(this[INTERNAL].transformerDirectories, finalOutputPath);
        ...
        try {
            const temp = new TemporaryCloudStorage(this[INTERNAL].params);
            for (const file of this[INTERNAL]._tempCloudStorageFiles) {
