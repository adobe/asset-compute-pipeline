...
const { Utils, Metadata } = require('./utils');
...
class Engine {
     ...
     * This step currently requires the asset being donwloaded, either by
     * @param {*} src 
     */
    async getMetadata(source) {
        ...
        let asset;
        if ((meta.isVideo() || meta.isTextTranscriptable())
            && !source.path) {
            ...
            asset = await Storage.getSource(source, this[INTERNAL].context.baseDirectory, false);
    ...
    async refinePlan(plan, source, outputInstructions) {
        try {
            ...
            console.log(assetMetadata.metadata);
