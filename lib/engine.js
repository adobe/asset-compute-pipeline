...
const ALLOWED_USER_DATA_FIELDS = new Set([
    'assetUuid', 'assetPath', 'eventHandlerId', 'repositoryId', 'name', 'uploadToken', 'customPrompts'
]);
...
class Engine {
    ...
    async prepare(transformerContext) {
        ...
        debug("Test Preparing directories");
        ...
        const { firefallClientId, firefallClientSecret, firefallAuthCode, firefallTier } = this[INTERNAL].params;
        ...
        // assetComputeEnabledFeatureToggle is an array containing specific feature flags; its presence does not indicate whether asset compute is enabled.
        // and is passed as-is, without using it as a boolean flag for enablement
        if (this[INTERNAL].params.assetComputeEnabledFeatureToggle) {
            transformerContext.input.featureToggle = this[INTERNAL].params.assetComputeEnabledFeatureToggle;
        }

        const { azureOpenAiApiKey, imsClientId, imsClientSecret, imsCode } = this[INTERNAL].params;
        ...
        // Add customPrompts to userData
        if (this[INTERNAL].params.renditions && this[INTERNAL].params.renditions[0]
            && this[INTERNAL].params.renditions[0].userData
            && this[INTERNAL].params.renditions[0].userData.customPrompts) {
            transformerContext.input.userData = transformerContext.input.userData || {};
            transformerContext.input.userData.customPrompts = this[INTERNAL].params.renditions[0].userData.customPrompts;
        }
