...
class Engine {
    ...
    async refinePlan(plan, source, outputInstructions) {
        try {
            ...
            // Problem: regardless of sourceType, we need to download the asset for getting metadata
            //          unless there's a way to get metadata without downloading the whole file.
            let transformerInput;
            console.log('here', source);
            if (source.path) {
                debug("asset path available : dont need to do anything");
                transformerInput = new Asset(source, source.path);
            } else {
                debug("asset path not available : download asset");
                this[INTERNAL].context.timers.download.start();
                // download input to <baseDirectory>/inputfile and set input.path
                transformerInput = await Storage.getSource(source, this[INTERNAL].context.baseDirectory, false);
                this[INTERNAL].context.timers.download.stop();
            }
            console.log(transformerInput);
            source.path = transformerInput.path;
