...
const {createBaseDirectory, createDirectories, cleanupDirectories} = require('./prepare');
const {getSource, putRendition} = require('./storage');
const {TemporaryCloudStorage} = require('./storage/temporary-cloud-storage');
const {GenericError, AssetComputeEvents, AssetComputeMetrics, OpenwhiskActionName, Reason, ClientError, AssetComputeLogUtils, RenditionFormatUnsupportedError, RenditionTooLarge, SourceFormatUnsupportedError, SourceUnsupportedError, SourceCorruptError} = require('@adobe/asset-compute-commons');
...
class Engine {
    ...
    wrapError(sourceError, message, location) {
        if (!(sourceError instanceof GenericError) 
            && !(sourceError instanceof ClientError)
            && !(sourceError instanceof RenditionFormatUnsupportedError)
            && !(sourceError instanceof RenditionTooLarge)
            && !(sourceError instanceof SourceFormatUnsupportedError)
            && !(sourceError instanceof SourceUnsupportedError)
            && !(sourceError instanceof SourceCorruptError)) {
    ...
    async prepare(transformerContext) {
        ...
        if (input.sourceType === TransformerSourceType.URL) {
            if (input.url && (protocol === 'https:')) {
                ...
                transformerInput = await getSource(input, transformerDirectory.base, true);
            } else {
            ...
            if (input.path) {
                ...
                transformerInput = await getSource(input, transformerDirectory.base, false);
