...
const { createBaseDirectory, createDirectories, cleanupDirectories } = require('./sdk/prepare');
const Asset = require('./sdk/asset');
const Rendition = require('./sdk/rendition');
const { getSource, putRendition } = require('./sdk/storage');
const { TemporaryCloudStorage } = require('./sdk/storage/temporary-cloud-storage');
...
const { GenericError, AssetComputeEvents, AssetComputeMetrics, OpenwhiskActionName, Reason, ClientError, AssetComputeLogUtils } = require('@adobe/asset-compute-commons');

// get utils
const utils = require('./sdk/utils/utils');
const Timer = require('./sdk/utils/timer');
...
class Engine {
    constructor(params) {
        ...
        this[INTERNAL].context = new EngineContext(this);
        this[INTERNAL].context.baseDirectory = createBaseDirectory(); // base directory for all transformer directories
        this[INTERNAL].context.transformerDirectories = []; // list of all the transformer directories
        this[INTERNAL].context._tempCloudStorageFiles = []; // list of files created in temporary cloud storage to be removed during clean up
        ...
        // metrics
        this[INTERNAL].metrics = this[INTERNAL].params.metrics || new AssetComputeMetrics(params);

        // rendition errors
        this[INTERNAL].renditionErrors = [];

        this[INTERNAL].processingStartTime = this[INTERNAL].params.times.gateway ?
            new Date(this[INTERNAL].params.times.gateway) :
            new Date(this[INTERNAL].params.times.process);
        ...
        this[INTERNAL].metrics.add({
            ...
            gatewayToProcessDuration: utils.durationSec(this[INTERNAL].params.times.gateway, this[INTERNAL].params.times.process),
            processToCoreDuration: utils.durationSec(this[INTERNAL].params.times.process, this[INTERNAL].params.times.core)
        });
    ...
    wrapError(sourceError, message, location) {
        if (!(sourceError instanceof GenericError) && !(sourceError instanceof ClientError)) {
    ...
    async run(plan) {
        ...
        try {
            ...
            await this[INTERNAL].metrics.handleError(error);
    ...
    async executeTransformer(plan) {
        const transformerName = plan.current.name;
        const transformerContext = new TransformerContext(plan, plan.current, this[INTERNAL].registry[transformerName]);
    ...
    async refinePlan(plan, source, outputInstructions) {
        try {
            ...
            this[INTERNAL].timers.processingTime.stop();

            // worker tests in the cli rely on these errors being in renditionErrors
            this[INTERNAL].renditionErrors.push(error);
            await this[INTERNAL].metrics.handleError(error, {
                ...
                metrics: {
                    ...
                    // durations
                    processingTime: this[INTERNAL].timers.processingTime.currentDuration()
                }
            });
    ...
    async prepare(transformerContext) {
        ...
        transformerContext.directory = await createDirectories(`${this.stepCount}-${transformer.name}`, this[INTERNAL].context.baseDirectory);
        this[INTERNAL].context.transformerDirectories.push(transformerContext.directory);
        ...
        transformerContext.output = new Rendition(output, transformerContext.directory.base);
        /**
         * For 1st sdk call
         * input: {
                type: 'image/png',
         ...
         */
        // if transformer.disableSourceDownload (defaults to false) then set it on input
        const disableInputDownload = transformer.options && transformer.options.disableSourceDownload;
        debug('disableInputDownload : ', disableInputDownload);
        const generateUrl = (input && input.sourceType === TransformerSourceType.URL)
            || input.generateUrl; // input.generateUrl will go away

        const downloadAsset = !disableInputDownload && !generateUrl;

        // create input object (type Asset)
        let transformerInput = {};
        // asset needs to be downloaded locally
        if (downloadAsset && input.url && !input.path) {

            this[INTERNAL].timers.download.start();

            // download input to <baseDirectory>/<stepNumber>-<transformerName>/inputfile and set input.path
            transformerInput = await getSource(
                input.url,
                transformerContext.directory.base,
                false
            );

            this[INTERNAL].timers.download.stop();

        } // url to asset is available and transformer to use it to process asset (ex: worker-ffmpeg)
        else if (!downloadAsset && input.url) {
            // dont download - T.compute will use url to process asset
            // input.url is already set and is passed on to Asset object 
            debug(`asset url available and disableInputDownload is ${disableInputDownload} : dont need to do anything`);
            transformerInput = new Asset(input);
        } // if (file exists locally && (T.manifest.input.sourceType === 'URL') then upload to tmp cloud
        else if (!downloadAsset && input.path) {
            const preSignedUrl = await this.getPresignedUrl(input.path);
            // TODO: remove this log
            debug('generated presigned url', preSignedUrl);
            input.url = preSignedUrl;
            transformerInput = new Asset(input, input.path);

        } // asset is an intermediate rendition from previous transformer and is already available in input.path
        else if (input.path) {
            transformerInput = new Asset(input, input.path);
        }

        if (!transformerInput || !transformerInput.path) {
            throw new GenericError('No source file accessible.');
        }
        transformerContext.input = transformerInput;
        transformerContext.input.auth = this[INTERNAL].params.auth;
    }

    async getPresignedUrl(source) {
        /**
         * if is presigned, return as-is,
        ...
        if (source.startsWith("https://")) {
            ...
            const temp = new TemporaryCloudStorage(this[INTERNAL].params);
            const path = temp.createUniqueName();
            // Upload
            await temp.upload(source, path);
            // Presign url
            const presignUrl = await temp.generatePresignURL(path, "rd");
            // Return url
            this[INTERNAL].context._tempCloudStorageFiles.push(presignUrl);
            return presignUrl;
        }
    }
    ...
    async upload(output) {
        try {
            ...
            // we should implement something similar to what we did for disableSourceDownload:
            // - remove this option and define it in the manifest.inputs as `sourceType` = `URL` or `LOCAL`
            debug('output rendition: %s', AssetComputeLogUtils.redactUrl(output));

            this[INTERNAL].timers.upload.start();

            await putRendition(output);

            this[INTERNAL].timers.upload.stop();
    ...
    async cleanup(finalOutput) {
        ...
        const cleanupSuccess = await cleanupDirectories(this[INTERNAL].context.transformerDirectories, finalOutputPath);
        ...
        this[INTERNAL].metrics.add({
            // durations
            processingTime: this[INTERNAL].timers.processingTime.totalDuration(),
            downloadDuration: this[INTERNAL].timers.download.totalDuration(),
            uploadDuration: this[INTERNAL].timers.upload.totalDuration()
        });

        try {
            const temp = new TemporaryCloudStorage(this[INTERNAL].params);
            for (const file of this[INTERNAL].context._tempCloudStorageFiles) {
                await temp.cleanUp(file);
            }
        } catch (error) {
            debug(`removing temporary cloud storage files failed: ${error.message || error}`);
        }
    ...
    getResult(output, err) {
        ...
        if (this[INTERNAL].renditionErrors.length > 0) {
            result.renditionErrors = this[INTERNAL].renditionErrors;
        }
    ...
    async renditionFailure(rendition, err, skipMetrics) {
        ...
        this[INTERNAL].renditionErrors.push(err);
        ...
        if (!skipMetrics) {
            ...
            await this[INTERNAL].metrics.handleError(err, {
                ...
                metrics: {
                    ...
                    // durations
                    processingTime: this[INTERNAL].timers.processingTime.currentDuration(),
                    downloadDuration: this[INTERNAL].timers.download.currentDuration(),
                    uploadDuration: this[INTERNAL].timers.upload.currentDuration(),
                    renditionDuration: utils.durationSec(this[INTERNAL].processingStartTime, renditionDoneTime)
                }
            });
        }
    }
    ...
    async renditionSuccess(rendition) {
        ...
        // send metrics
        await this[INTERNAL].metrics.sendMetrics(METRIC_RENDITION, {
            // rendition instructions
            ...instructions,
            renditionName: instructions.name,
            renditionFormat: instructions.fmt,
