...
//const http = require('./sdk/storage/http');
...
class Engine {
    ...
    async refinePlan(plan, source, outputInstructions) {
        try {
            ...
            const planFinder = new PlanFinder(this[INTERNAL].registry);
            const newSteps = await planFinder.findBestPlan(source, outputInstructions);
            if (!newSteps) {
            ...
            await this[INTERNAL].metrics.handleError(error, {
                location: `${this.actionName}_refinePlan`,
                metrics: {
    ...
    async prepare(transformerContext){
        ...
        const transformerDirectory = await createDirectories(`${this.stepCount}-${transformer.name}`, this[INTERNAL].baseDirectory);
        this[INTERNAL].transformerDirectories.push(transformerDirectory);
