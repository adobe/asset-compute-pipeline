...
const { getSource, putRendition, isDataUrlAndValid } = require('./sdk/storage');
...
class Engine {
    ...
    async prepare(transformerContext){
        ...
        if (input.sourceType === TransformerSourceType.URL) {
            ...
            if (input.url && validUrl.isHttpsUri(inputUrl)) {
                debug(`asset url available : dont need to do anything`);
                transformerInput = new Asset(input);
            } else if (input.url && isDataUrlAndValid(inputUrl)) {
                ...
                try {
                    ...
                    console.log(error);
                    throw new GenericError(`Fail generating presigned url.`);
                }
            }
