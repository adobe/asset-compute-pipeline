...
class Engine {
    ...
    async prepare(transformerContext){
        ...
        if (input && !input.url && !input.path) throw new GenericError('No source file accessible.');

        debug(`sourceType: ${input.sourceType}`);
        if (input.sourceType === TransformerSourceType.URL) {
            const protocol = URL.parse(input.url).protocol;
            if (input.url && protocol === HTTPS_PROTOCOL) {
                debug(`asset url available : dont need to do anything`);
                transformerInput = new Asset(input);
            } else if (input.url && protocol === DATA_PROTOCOL) {
                debug(`asset url available as datauri : upload asset to a temp cloud storage and generate a presigned URL`);
                // the will write asset locally and upload asset to a temp cloud storage to get a presigned URL
                this[INTERNAL].timers.upload.start();
                transformerInput = await getSource(input.url, transformerDirectory.base, true);
        ...
        } else { // LOCAL otherwise
            if (input.path) {
                debug(`asset path available : dont need to do anything`);
                transformerInput = new Asset(input, input.path);
            } else {
                debug(`asset path not available : donwload asset`);
                ...
                transformerInput = await getSource(input.url, transformerDirectory.base, false);
